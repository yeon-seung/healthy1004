package org.kosta.healthy.config.security;
...
@Configuration
@EnableWebSecurity
//@Secured, @PreAuthorize, @PostAuthorize 애노테이션을 사용하여 인가 처리를 하고 싶을때 사용하는 옵션
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	//비밀번호 암호화를 위한 bean 생성 -> MemberService에서 비번 암호화를 위해 사용,
	//MemberAuthenticationProvider에서 비번 일치여부를 위해 사용
	@Bean
	...
	protected void configure(HttpSecurity http) throws Exception {		
		 ...
		 */
		http.authorizeRequests() .antMatchers("/", "/home", "/fullcalendar/**", "/healthy/**", "/member/**", "/imgUpload/**") .permitAll().anyRequest().authenticated();
